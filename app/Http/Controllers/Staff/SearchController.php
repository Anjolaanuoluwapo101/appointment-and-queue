 ...
 * phone, or ID; appointments by ID; queue entries by number; plus
    ...
    public function search(Request $request): Response
    {
        ...
        if ($q !== '') {
            ...
            $appointments = is_numeric($q)
                ? Appointment::forHospital($hospitalId)
                    ->where('id', (int) $q)
                    ->with(['patient:id,full_name', 'department:id,name'])
                    ->limit(25)
                    ->get()
                : [];
        ...
        if (($validated['department_id'] ?? null) || ($validated['practitioner_id'] ?? null)
            ...
            $appointments = Appointment::forHospital($hospitalId)
                ...
                ->with(['patient:id,full_name', 'department:id,name', 'practitioner:id,full_name'])
